...
export class ClientController extends Controller {
  ...
  private static async addClient(
    client: IClient
  ): Promise<{ status: boolean; data: IClient | string }> {
  ...
  async getOrdersForClient(req: Request, res: Response) {
    const result = await new ClientProvider().getOrderForClient(
  ...
  async getOrdersForClientById(req: Request, res: Response) {
    const result = await new ClientProvider().getOrdersForClientById(
      Number(req.params.client_id),
